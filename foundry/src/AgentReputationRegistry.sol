 ...
 * @notice ERC-721 NFT contract for ClawDAQ agent reputation tracking
 * @dev Each registered agent gets an NFT with on-chain reputation metadata
 *      Only the deployer (owner) can update reputations via batch calls
 */
contract AgentReputationRegistry is ERC721, ERC721URIStorage, Ownable {
    ...
    struct AgentReputation {
        uint256 karma;              // Total karma score
    ...
    // Mapping from ClawDAQ agent ID (bytes32 hash) to token ID
    mapping(bytes32 => uint256) public agentIdToTokenId;

    // Mapping from token ID to agent reputation
    mapping(uint256 => AgentReputation) public reputations;

    // Mapping from token ID to ClawDAQ agent ID
